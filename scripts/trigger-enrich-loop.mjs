...
for (const arg of process.argv.slice(3)) {
  ...
  if (md) {
    ...
    if (!['null-price', 'null-model', 'null-locality', 'null-description', 'unenriched', 'unenriched-newest'].includes(md[1])) {
      console.error(`invalid --mode='${md[1]}' (expected null-price|null-model|null-locality|null-description|unenriched|unenriched-newest)`);
      process.exit(2);
    }
    mode = md[1];
  }
